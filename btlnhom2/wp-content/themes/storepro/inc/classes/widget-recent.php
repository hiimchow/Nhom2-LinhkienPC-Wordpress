 ...
 * @since      1.0.0
 */
class StorePro_Recent_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-storepro-recent posts-thumbnail-widget',
			'description' => __( 'Display recent posts with thumbnails.', 'storepro' )
		);

		// Create the widget.
		parent::__construct(
			'storepro-recent',                                   // $this->id_base
			__( '&raquo; Recent Posts Thumbnails', 'storepro' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		if ( false === ( $recent = get_transient( 'storepro_recent_widget_' . $this->id ) ) ) {
			...
			// Store the transient.
			set_transient( 'storepro_recent_widget_' . $this->id, $recent );
	...
	function flush_widget_transient() {
		delete_transient( 'storepro_recent_widget_' . $this->id );
	}
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'     => esc_html__( 'Recent Posts', 'storepro' ),
			'limit'     => 5,
			'show_date' => false
		);
