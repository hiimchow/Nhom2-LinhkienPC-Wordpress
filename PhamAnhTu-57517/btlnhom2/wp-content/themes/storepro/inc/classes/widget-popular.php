 ...
 * @since      1.0.0
 */
class StorePro_Popular_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-storepro-popular posts-thumbnail-widget',
			'description' => __( 'Display popular posts by comments with thumbnails.', 'storepro' )
		);

		// Create the widget.
		parent::__construct(
			'storepro-popular',                                   // $this->id_base
			__( '&raquo; Popular Posts Thumbnails', 'storepro' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		if ( false === ( $popular = get_transient( 'storepro_popular_widget_' . $this->id ) ) ) {
			...
			// Store the transient.
			set_transient( 'storepro_popular_widget_' . $this->id, $popular );
		...
		if ( $popular ) {
			echo '<ul>';
				...
				foreach ( $popular as $post ) :
					...
					echo '<li>';
						echo '<a href="' . esc_url( get_permalink( $post->ID ) ) . '" rel="bookmark">' . get_the_post_thumbnail( $post->ID, 'storepro-thumb', array( 'class' => 'entry-thumb', 'alt' => esc_attr( get_the_title( $post->ID ) ) ) ) . '</a>';
	...
	function flush_widget_transient() {
		delete_transient( 'storepro_popular_widget_' . $this->id );
	}
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'     => esc_html__( 'Popular Posts', 'storepro' ),
			'limit'     => 5,
			'show_date' => false
		);
	...
	?>
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id( 'show_date' ); ?>">
				<?php _e( 'Display post date?', 'storepro' ); ?>
			</label>
		</p>
